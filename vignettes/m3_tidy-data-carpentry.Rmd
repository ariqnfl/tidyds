...
`table1` termasuk "Tidy data" karena memisahkan semua data sesuai kolom kolomnya

```{r}
table2
```

`table2` tidak termasuk "Tidy data" karena menggabungkan case dan population
...
`table3` tidak termasuk "Tidy data" karena menggabungnkan dua kolom pada rate
...
`table4a` tidak termasuk "Tidy data" karena menjadikan 1999 dan 2000 sbg kolom

```{r}
table4b
```

`table4b` tidak termasuk "Tidy data" karena sama seperti diatas

```{r}
table5
```

`table5` termasuk "Tidy data" karena memisahkan semua kolom
...
karena spread dapat memisahkan tabel yang lurus menjadi menyamping
...
```{r}
table4a_tidy <- gather(table4a, key = "tahun", value = "cases", `1999`:`2000`,convert = TRUE)
#table4a_tidy <- gather(table4a, key = "tahun", value = "cases", 2:3)
table4a_tidy
```
...
tb4 <- summarise(tb3, cases = sum(case, na.rm = TRUE)) #na remove
tb5 <- ungroup(tb4)
...
1. membuat kolom baru yang bernama key yang memuat nama kolom new_sp_m014:newrel_f65 dan membuat kolom case yang memuat isi dari kolom new_sp_m014:newrel_f65
2. memilih kolom country,year dan case
3. membuat grup order berdasarkan country dan year
4. menjumlahkan case dan di assign di kolom cases dan menghapus case yang bernilai NA
5. meng ungroup data
6. membuat left join dengan dataset population
7. memfilter data yang memiliki populasi 0
8. membuat kolom baru dan mengisi kolom baru tersebut dengan 100*cases/population
...
perbedaan spread memakai kolom yang ada, gather membuat kolom baru
