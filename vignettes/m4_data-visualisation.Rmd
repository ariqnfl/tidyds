...
```{r}
qplot(x = carat , y = price, colour = color , data = diamonds)
```
...
```{r}
ggplot(un_smp, aes(x =bahasa_indonesia, y = matematika)) + geom_point()
```
...
```{r}
ggplot(un_smp,aes(x = ipa, y = bahasa_inggris)) + geom_point(aes(colour = status, size = jumlah_peserta),alpha = 0.2)

ggplot(un_smp,aes(x = ipa, y = bahasa_inggris))+geom_smooth()
```
