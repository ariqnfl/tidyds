...
```{r}
"Halo, nama saya adalah Ariq" # isi dengan nama Anda
```
...
# jika menggunakan vector hanya membaca type yang ada diawal
```
...
?mtcars # datasets reference?
?iris
dim(iris) # melihat jumlah baris dan kolom pada suatu dataset
str(iris) # melihat structure data iris
colnames(iris) # melihat nama kolom
head(iris) # melihat 5 data di awal
head(iris, 10) # melihat 10 data di awal
tail(iris) # melihat 5 data di akhir
tail(iris, 10) # melihat 10 data di akhir
summary(iris) # ___
iris[1, ] # ___
iris[c(1, 6, 12),] # ___
iris[ ,2] # ___
